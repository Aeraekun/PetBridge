 ...
 *  Refresh Token이 탈취당하면 Access Token을 계속 생성할 수 있기 때문에, RTR 방식 적용
 */

/**
 * OncePerRequestFilter를 상속받아 모든 요청마다 한 번씩 실행되는 필터를 정의.
 */
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    // doFilterInternal: 각 요청마다 필터링 작업을 수행.
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if (request.getRequestURI().equals(NO_CHECK_URL)) {
            filterChain.doFilter(request, response); // "login" 요청에 대해선, 다음 filter 호출해서 넘어가게 함
            return;
        }
        ...
        // Refresh Token이 DB의 RefreshToken과 일치하는지 판단 후, 일치한다면 AccessToken을 "재발급"해준다.
     ...
     * 그 유저 객체를 saveAuthentication()으로 인증 처리하여, 인증 허가 처리된 유저 객체를 SecurityContextHoder에 담기
     * 그 후 다음 인증 필터로 진행
     */
    ...
    public void saveAuthentication(User myUser) {
        ...
        if (password == null) { // 소셜 로그인 유저의 비밀번호 임의로 만들어서, 소셜 로그인 유저도 인증되도록 설정
